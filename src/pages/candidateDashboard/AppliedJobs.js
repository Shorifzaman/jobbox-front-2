  ...
  return (
    <div className="mx-2">
      ...
      <div className="flex items-center space-x-4 py-3">
        ...
        <button
          onClick={() => dispatch(toggleJobFilter("frontend"))}
          className={`px-4 py-2 border border-primary text-primary rounded-full hover:bg-primary/80 hover:text-white ${
            jobsFilteredBy.includes("frontend") && activeClass
          }`}
        >
          Frontend Developer
        </button>
      ...
      <div className="grid grid-cols-2 gap-5 pb-5">
        {data?.data
          ...
          ?.filter((job) => {
            if (jobsFilteredBy.includes("frontend")) {
              return job.position.toLowerCase().includes("frontend");
            } else {
              return job;
            }
          })
